...
def plot_list(city_file, boundary, origins, buildings, which_plots, metric_crs=2056):
    ...
    if "travel_time_matrix_center" not in gpd.list_layers(f"data/gpkg/{city_file}_data.gpkg")["name"].values and "travel_time_matrix_school" not in gpd.list_layers(f"data/gpkg/{city_file}_data.gpkg")["name"].values:
...
def plotter(city_file, to_plot, transit_lines):
    ...
    for matrix in to_plot:
        ...
        for col in ["ratio_p5", "ratio_p50", "ratio_p95", "spread"]:
            ...
            plt.savefig(f"plots/{city_file}_{matrix["plot_type"].dropna().iloc[0]}_{col}.pdf", bbox_inches="tight", dpi=300)           
